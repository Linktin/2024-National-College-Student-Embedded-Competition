...
from util.image_proc import FaceDataset

# from backbone.mobileFaceNet import MobileFaceNet
from backbone.mobileFaceNetV2 import MobileFaceNet

from loss.SFace import SFaceLoss
from sam.sam import SAM
torch.autograd.set_detect_anomaly(True)

from util.utils import separate_mobilefacenet_bn_paras
...
if __name__ == '__main__':
    ...
    if BACKBONE_NAME.find("MobileFace") >= 0:
    ...
    OPTIMIZER = optim.SGD([{'params': BACKBONE.parameters()}, {'params': HEAD.parameters()}], lr=LR, momentum=MOMENTUM, weight_decay=WEIGHT_DECAY)  # 定义优化器
    # 1. 首先定义基础优化器
    base_optimizer = optim.SGD
    # 3. 使用SAM替换SGD
    OPTIMIZER = SAM(
    [{'params': BACKBONE.parameters()}, {'params': HEAD.parameters()}],  # 模型参数
    ...
    for epoch in range(NUM_EPOCH):
        ...
        for inputs, labels in iter(trainloader):
            ...
            loss.backward()  # 第一次反向传播，不需要 retain_graph=True
            OPTIMIZER.first_step(zero_grad=True)#优化器优化参数

            # 第二次前向和反向传播用于 SAM
            features = BACKBONE(inputs)  # 重新计算特征
            outputs, loss, intra_loss, inter_loss, WyiX, WjX = HEAD(features, labels)
            prec1 = train_accuracy(outputs.data, labels, topk=(1,))
            intra_losses.update(intra_loss.data.item(), inputs.size(0))  # 类内损失
            inter_losses.update(inter_loss.data.item(), inputs.size(0))  # 类间损失
            ...
            if ((batch + 1) % DISP_FREQ == 0) and batch != 0:
                ...
                print('Epoch {} Batch {}\t'
                    ...
                    'Intra-class similarity {Wyi.val:.4f} ({Wyi.avg:.4f})\t'
                    'Inter-class similarity {Wj.val:.4f} ({Wj.avg:.4f})\t'
            ...
            if ((batch + 1) % VER_FREQ == 0) and batch != 0:  # perform validation & save checkpoints (buffer for visualization)
                ...
                with open("./logs/log.txt", "a") as file:
                    currented_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    # 将参数格式化为字符串，并写入文件
                    file.write('[%s]时间\n' % currented_time)
                    file.write('[%s][%d]XNorm: %1.5f\n' % (name, batch + 1, xnorm))
                    file.write('[%s][%d]Accuracy-Flip: %1.5f+-%1.5f\n' % (name, batch + 1, accuracy, std))
                    file.write('[%s][%d]Best-Threshold: %1.5f\n' % (name, batch + 1, best_threshold))     
                    file.write('-------------------------\n')

                del data_set#释放掉验证集内存    
